/* slot-text 0.3.1 (MIT, Daniel Belyi) — vendored as a classic global script.
   Source: https://textmotion.dev · npm "slot-text". Self-hosted, no build, no deps.
   Exposes window.slotText(el, initialText) -> { set, flash, value, destroy }. */
(function(){
/**
 ...
 */
function chromatic({ from = 0, spread = 320, saturation = 92, lightness = 60, } = {}) {
    return (index, total) => {
...
function clearSlotText(container, text = "") {
    settle(container);
    container.classList.remove("slot-text");
    container.textContent = text;
}
function slotText(element, initialText, options = {}) {
    ...
    return {
        ...
        flash(text, { revertAfter = 1400, enter, exit } = {}) {
            if (restingText === undefined) { restingText = value; }
            value = text;
        ...
        destroy() { clearTimeout(revertTimeout); clearSlotText(element, value); },
    };
}
